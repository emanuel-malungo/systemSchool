  ...
  static async updateTipoServico(id, data) {
    try {
        ...
        if (data[key] !== undefined && data[key] !== null && data[key] !== '') {
          switch (key) {
            ...
            case 'codigo_multa':
              ...
              break;
            case 'iva':
              // IVA é foreign key, 0 ou null deve ser null
              const ivaValue = parseInt(data[key]);
              updateData[key] = (!isNaN(ivaValue) && ivaValue > 0) ? ivaValue : null;
              break;
          }
        }
      });
      ...
      if (Object.keys(updateData).length === 0) {
        const existing = await prisma.tb_tipo_servicos.findUnique({
          where: { codigo: parseInt(id) },
          include: {
            tb_moedas: { select: { codigo: true, designacao: true } },
            tb_categoria_servicos: { select: { codigo: true, Designacao: true } }
        ...
        if (existing.tb_categoria_servicos) {
          existing.tb_categoria_servicos = {
            ...existing.tb_categoria_servicos,
            designacao: existing.tb_categoria_servicos.Designacao,
            Designacao: undefined
          };
          delete existing.tb_categoria_servicos.Designacao;
        }
        
        return existing;
      }

      const updated = await prisma.tb_tipo_servicos.update({
        where: { codigo: parseInt(id) },
        data: updateData,
        include: {
          tb_moedas: { select: { codigo: true, designacao: true } },
      ...
      if (error instanceof AppError) throw error;
      console.error('Erro detalhado ao atualizar tipo de serviço:', error);
      throw new AppError(`Erro ao atualizar tipo de serviço: ${error.message}`, 500);
    }
  }
