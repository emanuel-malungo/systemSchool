...
interface CreditNoteModalProps {
  ...
  onSuccess?: () => void;
  onRelaunchPayment?: (payment: PagamentoDetalhe) => void;
}
...
const CreditNoteModal: React.FC<CreditNoteModalProps> = ({
  ...
  const [relaunchAfterCancel, setRelaunchAfterCancel] = useState(false);
  ...
  useEffect(() => {
    if (open && payment) {
      ...
      setRelaunchAfterCancel(false); // Reset checkbox ao abrir
    }
  }, [open, payment]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const creditNoteData = {
        ...
        codigoPagamentoi: payment.codigo, // IMPORTANTE: Código do pagamento para excluir
      ...
      // Se marcou para relançar, chama o callback antes de fechar
      if (relaunchAfterCancel && onRelaunchPayment && payment) {
        onRelaunchPayment(payment);
      }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4">
        ...
        <div className="p-6 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Opção de Relançar Pagamento */}
            {onRelaunchPayment && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={relaunchAfterCancel}
                    onChange={(e) => setRelaunchAfterCancel(e.target.checked)}
                    className="mt-1 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                  <div>
                    <div className="flex items-center gap-2">
                      <RefreshCw className="h-4 w-4 text-blue-600" />
                      <span className="font-medium text-blue-900">Relançar Pagamento</span>
                    </div>
                    <p className="text-sm text-blue-700 mt-1">
                      Após anular, abrir automaticamente o formulário para registrar um novo pagamento 
                      para o mesmo aluno e mês ({payment.mes}).
                    </p>
                  </div>
                </label>
              </div>
