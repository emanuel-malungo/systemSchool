...
const findBestTipoServicoForAluno = (
  ...
  if (anoBuscar) {
    ...
    if (genericosCursoClasse.length > 0) {
      ...
      console.log('🎯 [FALLBACK] Usando genéricos CURSO+CLASSE');
    } else if (genericosCurso.length > 0) {
      candidatosFinais = genericosCurso;
      console.log('🎯 [FALLBACK] Usando genéricos SÓ CURSO');
    } else if (genericosCandidatos.length > 0) {
      candidatosFinais = genericosCandidatos;
      console.log('🎯 [FALLBACK] Usando OUTROS genéricos');
    }
...
const MakePaymentModal: React.FC<MakePaymentModalProps> = ({
  ...
  const extractAcademicData = useCallback(() => {
    ...
    if (confirmacoes && Array.isArray(confirmacoes) && confirmacoes.length > 0) {
      ...
      if (turmaData) {
        // Extrair curso e classe da designação da turma
        const turmaDesignacao = turmaData.designacao || '';
        const cursoExtraido = turmaDesignacao.split(' ').pop() || 'Não informado'; // Ex: "13ª Informática" -> "Informática"
        const classeExtraida = turmaDesignacao.split(' ')[0] || 'Não informada'; // Ex: "13ª Informática" -> "13ª"
        
        const dadosExtraidos = {
          curso: cursoExtraido !== 'Não informado' ? cursoExtraido : (matricula?.tb_cursos?.designacao || 'Curso não especificado'),
          classe: classeData?.designacao || classeExtraida,
  ...
  // Auto-selecionar ano letivo
  useEffect(() => {
    if (open && anosLectivos.length > 0 && !selectedAnoLectivo) {
      // Selecionar o ano letivo mais recente
  ...
  }, [open, anosLectivos, selectedAnoLectivo]);
  ...
  useEffect(() => {
    ...
    if (formData.codigo_Tipo_Servico > 0 && isAutoSelected) {
      return;
    }
    ...
    if (!anoLetivoObj) {
      console.log('❌ [AUTO-SELEÇÃO] Ano letivo não encontrado');
      return;
    }
    ...
    const tipoServicoEncontrado = findBestTipoServicoForAluno(
      tiposServico,
      anoLetivoObj,
      dadosParaSelecao
    );
        ...
        if (tipoServicoEncontrado) {
        console.log('✅ [AUTO-SELEÇÃO] Tipo de serviço encontrado:', tipoServicoEncontrado.designacao);
        console.log('💰 [AUTO-SELEÇÃO] Preço:', tipoServicoEncontrado.preco);
      
      setFormData(prev => ({
        ...prev,
        codigo_Tipo_Servico: tipoServicoEncontrado.codigo,
        preco: tipoServicoEncontrado.preco || 0
      }));
      setIsAutoSelected(true);
    } else {
      console.log('❌ [AUTO-SELEÇÃO] Nenhum tipo de serviço adequado encontrado');
      setIsAutoSelected(false);
    }
  }, [selectedAluno?.codigo, selectedAnoLectivo, tiposServico.length, anosLectivos, formData.codigo_Tipo_Servico, isAutoSelected, extractAcademicData]);
              ...
              {loadingTipos ? (
                ...
                (() => {
                  ...
                  return tipoSelecionado ? (
                    <div className="border-2 border-green-200 bg-green-50 rounded-lg">
                      <div className="p-3">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="ml-3 px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium flex items-center gap-1">
