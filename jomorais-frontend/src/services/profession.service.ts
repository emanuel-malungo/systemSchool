...
class ProfessionService {
   ...
   * @param data - Dados da profissão (designacao)
   * @returns Promise com a profissão criada
   */
  async createProfession(data: { designacao: string }): Promise<Profession> {
    try {
      const response = await api.post<ApiResponse<Profession>>(this.baseURL, data)
      
      if (response.data?.success) {
        return response.data.data
      }
      
      throw new Error(response.data?.message || 'Erro ao criar profissão')
    } catch (error) {
   ...
   * @returns Promise com a profissão atualizada
   */
  async updateProfession(id: number, data: { designacao: string }): Promise<Profession> {
    try {
      const response = await api.put<ApiResponse<Profession>>(`${this.baseURL}/${id}`, data)
      
      if (response.data?.success) {
        return response.data.data
      }
      
      throw new Error(response.data?.message || 'Erro ao atualizar profissão')
    } catch (error) {
      console.error(`Erro ao atualizar profissão com ID ${id}:`, error)
      throw error
    }
  }
  ...
  async deleteProfession(id: number): Promise<void> {
    try {
      ...
      if (!response.data?.success) {
        throw new Error(response.data?.message || 'Erro ao eliminar profissão')
      }
    } catch (error) {
      console.error(`Erro ao eliminar profissão com ID ${id}:`, error)
      throw error
    }
  }
}
