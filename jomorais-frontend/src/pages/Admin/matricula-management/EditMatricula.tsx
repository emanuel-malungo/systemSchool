...
export default function EditMatricula() {
	...
	const { data: matriculaData, isLoading: loadingMatricula } = useMatricula(Number(id))
	...
	const selectedAluno = searchResults.students.find(s => s.codigo === selectedAlunoId) ||
		(matriculaData?.data?.tb_alunos ? {
			codigo: matriculaData.data.tb_alunos.codigo,
			nome: matriculaData.data.tb_alunos.nome,
			sexo: matriculaData.data.tb_alunos.sexo,
			email: matriculaData.data.tb_alunos.email,
			telefone: matriculaData.data.tb_alunos.telefone
		} : undefined)

	const selectedCurso = searchResults.courses.find(c => c.codigo === selectedCursoId) ||
		(matriculaData?.data?.tb_cursos ? {
			codigo: matriculaData.data.tb_cursos.codigo,
			designacao: matriculaData.data.tb_cursos.designacao
		} : undefined)
	...
	} = useForm<EditMatriculaFormData>({
		resolver: yupResolver(editMatriculaSchema),
		mode: 'onChange',
		defaultValues: {
			codigo_Aluno: 0,
			codigo_Curso: 0,
	...
	useEffect(() => {
		if (matriculaData?.data) {
			const matricula = matriculaData.data

			setSelectedAlunoId(matricula.codigo_Aluno)
			setSelectedCursoId(matricula.codigo_Curso)
			setSearchAluno(matricula.tb_alunos?.nome || '')
			setSearchCurso(matricula.tb_cursos?.designacao || '')

			reset({
				codigo_Aluno: matricula.codigo_Aluno,
				codigo_Curso: matricula.codigo_Curso,
				data_Matricula: matricula.data_Matricula ? new Date(matricula.data_Matricula).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
				codigoStatus: matricula.codigoStatus,
			})
		}
	}, [matriculaData, reset])
	...
	const onSubmit: SubmitHandler<EditMatriculaFormData> = async (data) => {
		...
		try {
			const matriculaData: Partial<IMatriculaInput> = {
			...
			await updateMatricula.mutateAsync({ id: Number(id), matriculaData })
			...
			<Container>
				<div className="bg-white rounded-3xl border border-gray-100 p-20">
					<div className="flex flex-col items-center justify-center">
						...
						<p className="mt-6 text-sm font-bold text-[#1e293b] animate-pulse uppercase tracking-[0.2em]">Recuperando Matrícula...</p>
					</div>
				</div>
			</Container>
		)
		...
		return (
			<Container>
				<div className="bg-white rounded-3xl border border-gray-100 p-16 text-center">
					...
					<h2 className="text-2xl font-black text-[#1e293b] mb-2">Matrícula não encontrada</h2>
					<p className="text-gray-500 font-medium mb-8">O registro que você está tentando editar não foi localizado no sistema.</p>
					<Button
						onClick={() => navigate('/admin/student-management/enrolls')}
						className="bg-[#3964d7] hover:bg-[#2d52b2] px-8"
					>
						Voltar para a Lista
					</Button>
				</div>
			...
			<div className="space-y-8">
							...
							<div className="flex items-center gap-4">
								...
								<div>
									...
									<div className="flex items-center gap-2 mt-1">
										<span className="text-[10px] text-[#3964d7] font-black uppercase tracking-widest">Atualização Cadastral</span>
										<div className="w-1 h-1 bg-gray-300 rounded-full"></div>
										<span className="text-[10px] text-gray-400 font-bold">Matrícula #{id}</span>
									</div>
								</div>
							</div>
						</div>
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
					...
					<div className="bg-white rounded-3xl border border-gray-100 p-8 space-y-8">
						<div>
							<div className="flex items-center gap-3 mb-1">
								...
								<h2 className="text-[12px] font-black text-gray-400 uppercase tracking-[0.2em]">Ajuste de Matrícula</h2>
							</div>
							<p className="text-sm font-medium text-gray-500">Atualize o aluno, curso ou status conforme necessário</p>
						</div>
					...
					<div className="flex items-center justify-end gap-4 pt-4">
						...
						<Button
							type="submit"
							loading={isSubmitting || isUpdating || updateMatricula.isPending}
							disabled={isSubmitting || isUpdating || updateMatricula.isPending || !selectedAlunoId || !selectedCursoId}
